...
# Use curl_cffi for Medtronic API calls - bypasses TLS fingerprinting that blocks standard requests
try:
	from curl_cffi import requests as curl_requests
	_http = curl_requests.Session(impersonate="chrome120")
except ImportError:
...
def resolve_endpoint_config(discovery_url, is_us_region=False):
	discover_resp = json.loads(_http.get(discovery_url).text)
	sso_url = None
	...
	sso_config = json.loads(_http.get(sso_url).text)
...
def do_login(endpoint_config):
	...
	client_init_req = _http.post(client_init_url, data=data, headers=headers)
	...
	providers = json.loads(_http.get(authorize_url, params=auth_params).text) # this will redirect
	...
	reg_req = _http.post(reg_url, headers=reg_headers, data=csr)
	...
	token_req = _http.post(token_req_url, headers={"mag-identifier" : reg_req.headers["mag-identifier"]}, data=token_req_data)
