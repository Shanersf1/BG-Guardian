...
const MOBILE_USER_AGENT = 'CareLinkConnect/1.2.0 (iPhone; iOS 15.0; Scale/3.00)';
const MOBILE_ACCEPT = 'application/json';
  ...
  private async fetchAsCarepartner(_role: string): Promise<CareLinkData> {
    ...
    if (!patientId) {
      ...
      if (patientsResp.data?.length > 0) {
        const p = patientsResp.data[0];
        patientId = (p as { relativeId?: string }).relativeId ?? p.username;
    ...
    // 0) Try monitor v2 dashboard first (Bearer only, no mag-identifier)
    try {
      const dashboardResp = await this.axiosInstance.get<CareLinkData>(this.urls.monitorV2Dashboard, {
        headers: { 'User-Agent': MOBILE_USER_AGENT, 'Accept': MOBILE_ACCEPT, 'Role': 'carepartner' },
      });
      if (dashboardResp.data?.lastSG?.sg != null) {
        logger.log('GET data (monitor v2 dashboard)');
    ...
    // Try multiple API versions (v13 first for CareLink Connect app compatibility, then v11, v6, v5)
    const cloudHost = this.serverName.includes('.eu') ? 'clcloud.minimed.eu' : 'clcloud.minimed.com';
    const endpoints = [
      `https://${cloudHost}/connect/carepartner/v13/display/message`,
    ...
    for (const endpoint of endpoints) {
      try {
        const hasV13 = endpoint.includes('/v13/');
        const body: Record<string, string | undefined> = {
          username: this.options.username,
          role: 'carepartner',
          patientId,
        };
        if (hasV13) body.appVersion = '3.6.0';
        ...
        if (resp.status === 200 && resp.data) {
          const data = (resp.data as { patientData?: CareLinkData }).patientData ?? resp.data;
          if (data?.lastSG?.sg != null) {
            logger.log('GET data (as carepartner)', endpoint);
            return data as CareLinkData;
          }
        }
      } catch (e: unknown) {
        const err = e as { response?: { status: number; data?: unknown }; message?: string };
        const status = err.response?.status;
        const bodyPreview = err.response?.data != null
          ? (typeof err.response.data === 'string' ? err.response.data.slice(0, 200) : JSON.stringify(err.response.data).slice(0, 200))
          : '';
        logger.log('Endpoint failed:', endpoint, status ? `HTTP ${status}` : err.message, bodyPreview ? `— ${bodyPreview}` : '');
        console.log(`[Fetch] Carepartner endpoint failed: ${endpoint} — ${status ? 'HTTP ' + status : err.message || 'unknown'}${bodyPreview ? ' — ' + bodyPreview : ''}`);
      }
    }
